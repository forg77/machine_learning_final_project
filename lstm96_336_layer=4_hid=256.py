...
#       target shape torch.Size([16, 336, 7])
    ...
    if average_val_loss < 0.75:
        if average_val_loss < nowbest:
            torch.save(model.state_dict(), './lstm/Final_Rn_earlystop_Standard_scalar02_336_lstm_model_batch_64_lr_1e-2_layer=4_hid_256.pth')
            nowbest = average_val_loss
    t2 = time.time()
    print("time cost",t2-t1)
    print(f'Epoch [{epoch+1}/{num_epochs}], Train Loss: {loss.item():.4f}, Validation Loss: {average_val_loss:.4f}')
torch.save(model.state_dict(), './lstm/Final_Rn_Standard_scalar02_336_lstm_model_batch_64_lr_1e-2_layer=4_hid_256.pth')
