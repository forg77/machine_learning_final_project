...
for _ in range(EPOCH):
    ...
    for batch_seq, batch_target in dataloader:
        ...
        train_input_tgt = batch_seq[:,:,95:191]
    ...
    with torch.no_grad():
        for val_batch_data, val_batch_labels in val_loader:
            ...
            val_input_tgt = val_batch_data[:,:,95:191]
    ...
    if val_ave_loss < 0.65:
        if val_ave_loss < nowbest:
            ...
            torch.save(exp.state_dict(), '1_Final_Rn_earlystop_transformer_model_dim_64_96_batch_64_lr_1e-5_epoch=100.pth')
...
torch.save(exp.state_dict(), '1_Final_Rn_transformer_model_dim_64_96_batch_64_lr_1e-5_epoch=100.pth')
