...
for epoch in range(num_epochs):
    ...
    for batch_seq, batch_target in dataloader:
      ...
      # inputx = x
      # pred = torch.zeros((batch_size,0,7))
      # for _ in range(96):
      #   output,hc = model(inputx, '0')
      #   pred = torch.concat([pred,output.unsqueeze(1)],dim=1)
      #   inputx = torch.concat([inputx[:, 1:, ], output.unsqueeze(1)], dim=1)
    ...
    if average_val_loss < 0.9:
       if average_val_loss < nowbest:
        torch.save(model.state_dict(), './lstm/Final_Rn_earlystop_Standard_scalar01_lstm_model_batch_64_epoch_10_lr_1e-3_layer=1_hid_1024.pth')
...
torch.save(model.state_dict(), './lstm/Final_Rn_Standard_scalar01_lstm_model_batch_64_epoch_10_lr_1e-3_layer=1_hid_1024.pth')
